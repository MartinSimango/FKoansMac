...
module ``21: Sequences and Arrays`` =
    ...
    *)
    ...
    let ``04 Creating a sequence (Method 4).`` () =
        ...
        let rec hailstone x =
            seq {
                yield x // I'm giving back a generated value here //produce element
                match x with //check what element was
                | 1 -> ()
                | _ ->
                    let result =
                        match x%2 with
                        | 0 -> x/2
                        | _ -> 3*x+1
                    yield! hailstone result // I'm giving back values taken from a sequence here //procude element produced from futher calls
                    //yield! is for sequences
            }
        let rec puffery x =
            seq{
                yield x
